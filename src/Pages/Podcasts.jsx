...
import Header from "../Components/Core/Header/Header_New"; // Assuming path is correct
import { blogPosts } from "../data/podcastData"; // Import the podcast data

// The PodcastCard component now accepts an 'id' to create a unique link.
const PodcastCard = ({ id, imgSrc, title, description, host, date }) => (
  // The entire card is wrapped in a Link component.
  // Clicking anywhere on the card will navigate to the podcast's detail page.
  <Link
    to={`/podcasts/${id}`}
        ...
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-10">
            {blogPosts.map((episode) => (
              <PodcastCard
                key={episode.id}
                id={episode.id} // Pass the id for the link
                imgSrc={episode.imageUrl} // Use imageUrl to match data file
                title={episode.title}
                description={episode.content} // Use content for description
                host={episode.host}
                date={episode.date}
              />
            ))}
